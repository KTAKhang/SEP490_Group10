...
const authSalesStaffMiddleware = async (req, res, next) => {
  ...
  try {
    const decoded = jwt.verify(token, process.env.ACCESS_TOKEN_SECRET);
    const userData = await UserModel.findById(decoded._id).populate(
      "role_id",
      "name",
    );
    if (!userData || userData.currentAccessToken !== token) {
      return res.status(401).json({
        status: "ERR",
        message: "Your account has been logged in from elsewhere.",
      });
    ...
    return res
      .status(403)
      .json({ message: "Access denied. Sales staff only.", status: "ERR" });
  } catch (err) {
...
/**
 * Cho phép Admin hoặc warehouse staff (warehouse_staff / warehouse-staff).
 * Dùng cho xác nhận hoàn tiền refund (payment PENDING → SUCCESS).
 */
const authAdminOrWarehouseStaffMiddleware = async (req, res, next) => {
  try {
    const authHeader = req.headers?.authorization;
    if (!authHeader || !authHeader.startsWith("Bearer ")) {
      return res.status(401).json({
    ...
    const isWarehouse =
      ...
      (user.role_id?.name || "").toLowerCase() === "warehouse_staff";
    if (!isAdmin && !isWarehouse) {
      return res.status(403).json({
        status: "ERR",
        message: "Access denied. Admin or warehouse staff only.",
      });
    }
    req.user = user;
    next();
    ...
    if (error.name === "TokenExpiredError") {
      return res
        .status(401)
        .json({ status: "ERR", message: "Token has expired" });
    }
    if (error.name === "JsonWebTokenError") {
...
const authStaffOrAdminMiddleware = async (req, res, next) => {
  ...
  try {
    ...
    if (
      userData?.role_id?.name === "sales-staff" ||
    ...
    return res
      .status(403)
      .json({ message: "Access denied. Staff or admin only.", status: "ERR" });
  } catch (err) {
    return res.status(401).json({ message: "Invalid token", status: "ERR" });
  }
};
